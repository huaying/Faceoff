...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    var attackerSFAnimation = [SKTexture]()
    var heroSFAnimation = [SKTexture]()
    var attackerRGAnimation = [SKTexture]()
    var attackerLHWAnimation = [SKTexture]()
    var attackerRHLAnimation = [SKTexture]()
    ...
    func start() {
        ...
        self.initAttackerSFScene()
        self.initHeroSFScene()
        self.initAttackerRGScene()
        self.initAttackerLHWScene()
        self.initAttackerRHLScene()
    ...
    */
    ...
    func initAttackerSFScene() {
        let attackerSFAtlas = SKTextureAtlas(named: "spacemanbacksf")
        
        attackerSFAnimation = []
        
        for index in 1...attackerSFAtlas.textureNames.count {
            let imgName = String(format: "smbsf%02d", index)
            
            attackerSFAnimation += [attackerSFAtlas.textureNamed(imgName)]
        }
    }
    
    func initHeroSFScene() {
        let heroSFAtlas = SKTextureAtlas(named: "spacemanfrontsf")
        
        heroSFAnimation = []
        
        for index in 1...heroSFAtlas.textureNames.count {
            let imgName = String(format: "smfsf%02d", index)
            
            heroSFAnimation += [heroSFAtlas.textureNamed(imgName)]
        }
    }
    
    func initAttackerRGScene() {
        let attackerRGAtlas = SKTextureAtlas(named: "spacemanbackrgrevised")
        
        attackerRGAnimation = []
        
        for index in 1...attackerRGAtlas.textureNames.count {
            let imgName = String(format: "smbrg%02d", index)
            
            attackerRGAnimation += [attackerRGAtlas.textureNamed(imgName)]
        }
    }
    
    func initAttackerLHWScene() {
        let attackerLHWAtlas = SKTextureAtlas(named: "spacemanbacklhw")
        
        attackerLHWAnimation = []
        
        for index in 1...attackerLHWAtlas.textureNames.count {
            let imgName = String(format: "smblhw%02d", index)
            
            attackerLHWAnimation += [attackerLHWAtlas.textureNamed(imgName)]
        }
    }
    
    func initAttackerRHLScene() {
        let attackerRHLAtlas = SKTextureAtlas(named: "spacemanbackrhl")
        
        attackerRHLAnimation = []
        
        for index in 1...attackerRHLAtlas.textureNames.count {
            let imgName = String(format: "smbrhl%02d", index)
            
            attackerRHLAnimation += [attackerRHLAtlas.textureNamed(imgName)]
        }
    }
    ...
    func receiveRemoteData(notification: NSNotification){
        ...
        if let tappp = receivedData["tap"] as? Int{
            ...
            // hero animation
            let heroNode = self.childNodeWithName(FaceoffGameSceneChildName.HeroName.rawValue)
            
            if(heroNode != nil) {
                let animation = SKAction.animateWithTextures(heroSFAnimation, timePerFrame: 0.02)
                
                heroNode?.runAction(animation)
            }
        ...
        */
        ...
        if let index = receivedData["didAttack"] as? Int{
            ...
            if index == 0 {
                ...
                if(attackerNode != nil) {
                    let animation = SKAction.animateWithTextures(attackerRGAnimation, timePerFrame: 0.03)
                ...
                */
                ...
                if(attackerNode != nil) {
                    let animation = SKAction.animateWithTextures(attackerRHLAnimation, timePerFrame: 0.03)
        ...
        for touch in touches {
            ...
            if(fighting) {
                ...
                if attacker.containsPoint(location){
                    ...
                    if(attackerNode != nil) {
                        let animation = SKAction.animateWithTextures(attackerSFAnimation, timePerFrame: 0.02)
    ...
    func oneAttackCircle(type: Int, index: Int){
        ...
        if(attackerNode != nil) {
            let animation = SKAction.animateWithTextures(attackerLHWAnimation, timePerFrame: 0.03)
...
private extension GameScene {
    ...
    func loadAttacker(){
        ...
        attacker = SKSpriteNode(imageNamed: "attacker")
        attacker.name = "attackerNode"
        attacker.position = CGPoint(x: frame.midX * 0.5 + 120, y: frame.midY * 0.5 + 40)
        attacker.xScale = 0.50
        attacker.yScale = 0.50
    ...
    func loadHero() {
        ...
        hero.position = CGPoint(x: frame.midX * 1.5 + 5, y: frame.midY * 1.5 + 40)
        hero.xScale = 0.30
        hero.yScale = 0.30
            ...
            if(self.beam.position.y > (self.view?.bounds.size.height)!/2){
                ...
                self.beam.runAction(SKAction.sequence([SKAction.moveToY(0, duration: 0.5),SKAction.fadeAlphaTo(0, duration: 0.5)]), completion: {[unowned self] () -> Void in
                    ...
                    if self.selectedIndex == 0 {
                        ...
                        if(attackerNode != nil) {
                            let animation = SKAction.animateWithTextures(self.attackerLHWAnimation, timePerFrame: 0.03)
                            
                            attackerNode?.runAction(animation, completion: {
                        ...
                        if(attackerNode != nil) {
                            let animation = SKAction.animateWithTextures(self.attackerRGAnimation, timePerFrame: 0.03)
                        ...
                        if(attackerNode != nil) {
                            let animation = SKAction.animateWithTextures(self.attackerRHLAnimation, timePerFrame: 0.03)
